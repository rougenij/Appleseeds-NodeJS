...
const populateData = async (db) => {
  ...
  const timmy = await db
    .collection("users")
    .findOne({ email: "timmy123@email.com" });
  const mordi = await db
    .collection("users")
    .findOne({ email: "mordi@email.com" });
  ...
  const timmyPost = await db
    .collection("posts")
    .findOne({ authorID: timmy._id });
